...
export default function AddClientProject({ client, showModal, onHide }) {
  const statuses = [
  ...
  // Create new project with data for specific client
  const onSubmit = async (data) => {
    try {
      setApiError(null);
      await projectService.post({ ...data, clientId: client?.id }, client?.id);
      reset({ ...data });
    } catch (err) {
      ...
      <Modal.Header closeButton>
        <Modal.Title>
          <h4 className="p-0 m-0 modal-title">
            New Project for {client?.fullName}
          </h4>
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
